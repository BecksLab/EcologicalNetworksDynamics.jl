@testset "filtering simulation" begin
    ...
    @test_throws ArgumentError("The `last` argument, when given as a \
                               string, should end with character '%'") extract_last_timesteps(
        sim,
        last = "15%5",
    )

    @test_throws ArgumentError("Cannot extract -100.1% of the solution's timesteps: 0% < \
                               `last` <= 100% must hold.") extract_last_timesteps(
        sim,
        last = "-100.1%",
    )

    @test_throws ArgumentError("Cannot extract 0.0% of the solution's timesteps: 0% < \
                               `last` <= 100% must hold.") extract_last_timesteps(
        sim,
        last = "0%",
    )
    ...
    @test_throws ArgumentError(err_sp_msg) extract_last_timesteps(
        sim;
        last = "10%",
        idxs = "s3",
    )
    ...
    @test_throws ArgumentError(err_id_msg) extract_last_timesteps(
        sim;
        last = "10%",
        idxs = 4,
    )
...
@testset "check extinction" begin
    ...
    @test EcologicalNetworksDynamics.check_last_extinction(
        100;
        t = [100],
        species = ["s2"],
        last = 1,
    ) == true
    warn_msg = "With `last` = 2, a table has been extracted with the species [\"s2\"], \
    that went extinct at timesteps = [100]. Set `last` <= 1 to get rid of them."
    @test_warn warn_msg EcologicalNetworksDynamics.check_last_extinction(
        100,
        t = [100],
        species = ["s2"],
        last = 2,
