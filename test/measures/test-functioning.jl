@testset "Producer growth rate" begin
    ...
    params = ModelParameters(foodweb)
    sim = simulate(params, [0.5, 0.5])
    normal_growth = producer_growth(sim, last = 1, out_type = :all)

    @test length(normal_growth.s) == 1 

    # Test structure:
    avg_g = producer_growth(sim, last = 50, out_type = :mean)
    sd_g = producer_growth(sim, last = 50, out_type = :std)
...
@testset "Total biomass, species persistence, Hill numbers" begin
    ...
    sim_two_sp = simulate(params, [0.5, 0.5])
    sim_one_sp = simulate(params, [0, 0.5])
    sim_zero = simulate(params, [0, 0])

    # Total biomass should converge to K
    @test isapprox(total_biomass(sim_one_sp, last = 1), sim_one_sp.prob.p.environment.K[2], rtol = 0.001)
    ...
    @test BEFWM2.species_richness(sim_two_sp[:, end]) == foodweb_richness(sim_two_sp, last = 1) == 2
    @test BEFWM2.species_richness(sim_one_sp[:, end]) == foodweb_richness(sim_one_sp, last = 1) == 1
    @test BEFWM2.species_richness(sim_zero[:, end]) == foodweb_richness(sim_zero, last = 1) == 0

    # Other hill diversity numbers 
    ## Shannon 
    @test BEFWM2.shannon(sim_two_sp[:, end]) == foodweb_shannon(sim_two_sp, last = 1) == log(2) 
    @test BEFWM2.shannon(sim_one_sp[:, end]) == foodweb_shannon(sim_one_sp, last = 1) ≈ 0.0 # 0 entropy if 1 species only
    @test isnan(BEFWM2.shannon(sim_zero[:, end])) == isnan(foodweb_shannon(sim_zero, last = 1)) # Not defined for 0 species 

    ## Simpson
    @test BEFWM2.simpson(sim_two_sp[:, end]) == foodweb_simpson(sim_two_sp, last = 1) == 1 / sum( 2 .^ [1/2, 1/2])
    @test BEFWM2.simpson(sim_one_sp[:, end]) == foodweb_simpson(sim_one_sp, last = 1) == 1/ sum( 2 .^ 1) == 0.5# 0.5 if 1 species only
    @test isnan(BEFWM2.simpson(sim_zero[:, end])) == isnan(foodweb_simpson(sim_zero, last = 1)) # Not defined for 0 species 

    # Community evenness
    @test BEFWM2.pielou(sim_two_sp[:, end]) == foodweb_evenness(sim_two_sp, last = 1) ≈ 1.0 # Maximum equity of species biomass
    @test isnan(BEFWM2.pielou(sim_one_sp[:, end])) ==  isnan(foodweb_evenness(sim_one_sp, last = 1))# Should be NaN if 1 species
    @test isnan(BEFWM2.pielou(sim_zero[:, end])) ==  isnan(foodweb_evenness(sim_zero, last = 1))# Should be NaN if 0 species
