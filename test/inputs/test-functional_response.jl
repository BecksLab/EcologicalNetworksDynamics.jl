...
net_interference = MultiplexNetwork(foodweb_3sp, C_itf=1.0, intensity=(c=0, f=0, i=0, r=0))
net_refuge = MultiplexNetwork(foodweb_3sp, C_ref=1.0, intensity=(c=0, f=0, i=0, r=0))
...
@testset "Classic functional response parameters" begin
    # Default
    Fclassic_1 = ClassicResponse(foodweb_2sp)
...
@testset "Classic functional response functor" begin
    # Index by index - FoodWeb
    Fclassic1_fw = ClassicResponse(foodweb_2sp)
    ...
    # Non-default hill exponent
    Fclassic_1 = ClassicResponse(foodweb_2sp, h=3)
    F21 = (1 * 0.5 * 2^3) / (1 + 0.5 * 1 * 2^3)
    @test Fclassic_1(2) == sparse([0 0; F21 0])

    # Non-default attack rate
    Fclassic_1 = ClassicResponse(foodweb_2sp, aᵣ=0.2)
    ...
    # Consumer feeding on several resources
    Fclassic2_fw = ClassicResponse(foodweb_3sp)
    Fclassic2_nti = ClassicResponse(net_interference)
    B = [1, 1, 1] # uniform biomass distribution
    F21 = (1 * 0.5 * 1^2) / (1 + 0.5 * 1 * 1^2)
    ...
    Fclassic2_fw = ClassicResponse(foodweb_3sp, aᵣ=aᵣ) #...and non-uniform attack rate
    Fclassic2_nti = ClassicResponse(net_interference, aᵣ=aᵣ)
    F21 = (1 * 0.5 * 3^2) / (1 + 0.5 * 1 * 3^2)
    ...
    # Adding intraspecific interference
    Fclassic2_fw = ClassicResponse(foodweb_3sp, c=1)
    Fclassic2_nti = ClassicResponse(net_interference, c=1)
    B = [3, 2, 1] # non-uniform biomass distribution
