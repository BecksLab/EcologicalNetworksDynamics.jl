...
function setindex!(v::AbstractGraphDataReadWriteView, rhs, refs)
    ...
    try
        ...
        e isa CheckError && rethrow(
            V,
            "cannot set value $(inline(index, refs)):\n  \
               $(e.mess)\n\
             Received rhs: $(repr(rhs)) ::$(typeof(rhs)).",
        )
    end
    write!(v._graph, typeof(v), rhs, index)
end
...
write!(::Internal, ::Type{<:NodesWriteView}, rhs, index) = nothing

# Check the value about to be written.
check(::Internal, ::Type{<:NodesWriteView}, rhs, index) = nothing

# For use within custom methods extensions of the above.
struct CheckError
    message::String
end
checkerr(mess) = throw(CheckError(mess))
export checkerr
...
function check_dense_index(v::AbstractNodesView, ::Any, index::Tuple{Vararg{Int}})
